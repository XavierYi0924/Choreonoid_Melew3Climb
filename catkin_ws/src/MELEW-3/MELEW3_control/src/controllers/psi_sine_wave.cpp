...
class psi_sine_wave : public BaseController {
private:
  const double psi_frequency = 0.25;        // 周波数[Hz]
  const double psi_amplitude = M_PI / 6.0;  // 振幅[rad]
...
public:
  ...
  std::vector<double>          init_leg_yaw = {psi_init, psi_init, psi_init, psi_init};  // 初期足部YAW角度
 ...
 *
 */
psi_sine_wave::psi_sine_wave(void) {
 ...
 *
 */
void psi_sine_wave::control_loop(void) {
  ...
  // Move psi sine wave
    ...
    */
    ...
    leg_yaw_ref = {psi_amplitude * cos(2 * M_PI * psi_frequency * loop_time),
                   psi_amplitude * cos(2 * M_PI * psi_frequency * loop_time),
                   psi_amplitude * cos(2 * M_PI * psi_frequency * loop_time),
                   psi_amplitude * cos(2 * M_PI * psi_frequency * loop_time)};
  ...
  // Finalize
    ...
    ROS_INFO("Moving psi sine wave complete!");
    stop_controller();
  }
}
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "psi_sine_wave");
  // ROS_WARN("psi_sin in");
  MELEW3_control::BaseController* controller = new MELEW3_control::psi_sine_wave;
  ros::waitForShutdown();
  return 0;
}
