...
class low_posture : public BaseController {
private:
  const double settingTime = 5.0;   // 動作開始姿勢への遷移時間[s]
  const double movingTime = 10.0;   // 動作時間[s]
  const double x_init = 0.0;        // x軸ベースリンク初期位置[m]
  const double y_init = 0.0;        // y軸ベースリンク初期位置[m]
  const double z_init = 0.4 * cos(M_PI / 12) - 0.2  + 0.068;        // z軸ベースリンク初期位置[m]
 ...
 *
 */
void low_posture::control_loop(void) {
  double current_time = get_time();
  // Transition to initial pose
  ROS_WARN("zsin loop in");
  if (phase == 0) {
    if (pose_transition()) {
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "low_posture");
  // ROS_WARN("Low Posture in");
  MELEW3_control::BaseController* controller = new MELEW3_control::low_posture;
  ros::waitForShutdown();
  return 0;
}
