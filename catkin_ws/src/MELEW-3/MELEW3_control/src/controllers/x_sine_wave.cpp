...
class x_sine_wave : public BaseController {
private:
  const double x_frequency = 0.25;    // 周波数[Hz]
  const double x_amplitude = 0.05;    // 振幅[m]
  const double settingTime = 3.0;     // 動作開始姿勢への遷移時間[s]
  const double movingTime = 10.0;     // 動作時間[s]
  const double x_init = x_amplitude;  // x軸ベースリンク初期位置[m]
 ...
 *
 */
void x_sine_wave::control_loop(void) {
  ...
  if (phase == 1) {
    ...
    if (pose_transition(init_leg_pos, init_leg_yaw, init_COM_pos, init_COM_yaw, settingTime)) {
      ROS_INFO("phase1 end");
      ROS_WARN("x_amp: %f, x_freq: %f", x_amplitude, x_frequency);
      ROS_INFO("Enter key to start moving x sine wave");
      char buf;
      std::cin >> buf;
      phase = 2;
      initialize_flag = false;
  ...
  // Move x sine wave
    ...
    */
    ...
    COM_pos_ref = Eigen::Vector3d(x_amplitude * cos(2 * M_PI * x_frequency * loop_time), y_init, z_init);
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "x_sine_wave");
  // ROS_WARN("xsin in");
  MELEW3_control::BaseController* controller = new MELEW3_control::x_sine_wave;
  ros::waitForShutdown();
  return 0;
}
