...
private:
  ...
  Utils::Five_Spline five_spline_z_;
  ...
  /*** 経由点情報 ***/
  Utils::Five_Spline::point zi1;
  Utils::Five_Spline::point zi2;
...
void jumping::control_loop(void) {
  ...
  if (phase == 0) {
    if (pose_transition()) {
      ...
      ref.z_s = 0.2 * sqrt(2) + 0.136 / 2;
      ref.z_sq = 0.20 + 0.136 / 2;
      ref.z_f = 0.35 + 0.136 / 2;
      ref.z_l = ref.z_s + 0.1;
      ref.z_e = ref.z_s + 0.1;
      // ref.z_ref = 0.45 + 0.136 / 2;
      ref.z_ref = 0.50 + 0.136 / 2; 
      
      // Time Relationship
      timer.t_s           // start time           0.0s
        = 0.0;                  
      timer.t_rs          // running start time   3.0s
        = timer.t_s + 3.0; 
      timer.t_sqs         // squat start time     3.2s
        = timer.t_rs + 0.0;
      timer.t_sq          // squat time           3.7s
        = timer.t_sqs + 0.5; 
      timer.t_js          // jump start time      4.2s
        = timer.t_sq + 0.2; 
      timer.t_f           // flying time          
        = timer.t_js + 1.0; 
      timer.t_l           // landing time 
        = timer.t_f + sqrt(2 * (ref.z_ref - ref.z_f) / param.g) + sqrt(2 * (ref.z_ref - ref.z_l) / param.g); 
      timer.t_je          // jump end time
        = timer.t_l + 0.2; 
      timer.t_re          // running end time
        = timer.t_je + 0.8; 
      timer.t_e           // end time
        = timer.t_re + 2.0; 
      ...
      timer.c_sqs = timer.t_sqs / timer.dt;
      timer.c_sq = timer.t_sq / timer.dt;
      ...
      timer.l_ini = 0;
      timer.l_s = timer.t_s / timer.MPC_dt; 
      timer.l_rs = timer.t_rs / timer.MPC_dt; 
      timer.l_sqs = timer.t_sqs / timer.MPC_dt; 
      timer.l_sq = timer.t_sq / timer.MPC_dt; 
      timer.l_js = timer.t_js / timer.MPC_dt; 
      timer.l_f = timer.t_f / timer.MPC_dt; 
      timer.l_l = timer.t_l / timer.MPC_dt; 
      timer.l_je = timer.t_je / timer.MPC_dt; 
      timer.l_re = timer.t_re / timer.MPC_dt; 
      timer.l_e = timer.t_e / timer.MPC_dt; 
  ...
  if(timer.current_count < timer.c_e){
    ...
    ZMP_pos_ref = Eigen::Vector3d(ref.ZMP[timer.loop_count].x(), 0.0, 0.0);
    COM_pos_ref = Eigen::Vector3d(ref.CoM[timer.loop_count].x(), 0.0, ref.CoM[timer.current_count].z());
    COM_vel_ref = Eigen::Vector3d(ref.dCoM[timer.loop_count].x(), 0.0, ref.dCoM[timer.current_count].z());
    COM_acl_ref = Eigen::Vector3d(ref.ddCoM[timer.loop_count].x(), 0.0, ref.ddCoM[timer.current_count].z());
    ...
    // std::cout << "loop_count : " << timer.loop_count << std::endl;
    timer.current_count ++;
    if(timer.current_count%5 == 0) timer.loop_count ++;
  }
  else {
...
double jumping::JumpPointer(Param param_, Reference ref_) {
  param = param_;
  ref = ref_;
...
void jumping::ZTG(){
  ...
  // 1点目の時刻，位置，速度，加速度を指定 (For Squat)
  z1_ = {timer.t_sqs, 
        ref.z_s, 
        0.0, 
        0.0 };
  // 2点目の時刻，位置，速度，加速度を指定 (For Squat)
  z2_ = {timer.t_sq, 
        ref.z_sq, 
        0.0, 
        0.0  };
  ...
  // 経由点の情報をまとめたベクトルを定義
  std::vector<Utils::Five_Spline::point> points_z_1_ = {z1_, z2_};
  std::vector<Utils::Five_Spline::point> points_z_1 = {z1, z2, z3, z4};
  // 5次多項式補間の係数を計算
  five_spline_z_.Calc_Spline(points_z_1_); 
  five_spline_z.Calc_Spline(points_z_1); 


  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    if(period_count < timer.c_sqs){
      com_z_tra(period_count) = z1_.x;
      com_dz_tra(period_count) = z1_.dx;
      com_ddz_tra(period_count) = z1_.ddx;
    ...
    else if(period_count < timer.c_sq){
      com_z_tra(period_count) = five_spline_z_.Get_Spline_x(period_count*timer.dt);
      com_dz_tra(period_count) = five_spline_z_.Get_Spline_dx(period_count*timer.dt);
      com_ddz_tra(period_count) = five_spline_z_.Get_Spline_ddx(period_count*timer.dt);
      ref.CoM[period_count].z() = com_z_tra(period_count);
      ref.dCoM[period_count].z() = com_dz_tra(period_count);
      ref.ddCoM[period_count].z() = com_ddz_tra(period_count);
    }
    else if(period_count < timer.c_js){
...
void jumping::FTG(){
  ...
  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    ...
    // std::cout << "Count " << period_count << "-> ref.FL_foot[].x() : " << ref.FL_foot[period_count].x() << std::endl;
  }
  //** FTG End **//////////////////////////////////////////////////////////////////////////////
}
...
void jumping::XTG(){
    ...
    while (1){
        timer.loop_count++;
        timer.current_time = timer.current_time + timer.MPC_dt;

        // 終了条件
        if( timer.loop_count == timer.l_rs){
            break;
        }

    }
    
    // 接地期(current_count == timer.c_sからスタート)
    VHIP_MPC_PG mpc;
  
    // timer.current_count番目の目標値
    double x_k, dx_k, ddx_k, dddx_k, zmp_k; 
    x_k = 0.0;
    dx_k = 0.0;
    ddx_k = 0.0;
    dddx_k = 0.0;
    ...
    while(1){
        ...
        std::cout << "MPC " << (int)(((timer.current_time - timer.t_rs) / (timer.t_f - timer.t_rs)) * 100.0) << "% complete" << std::endl;

        // timer.current_count番目の目標値を格納
        ref.CoM[timer.loop_count].x() = x_k;
        ref.dCoM[timer.loop_count].x() = dx_k;
        ref.ddCoM[timer.loop_count].x() = ddx_k;
        ref.dddCoM[timer.loop_count].x() = dddx_k;
        ref.ZMP[timer.loop_count].x() = zmp_k;
        // std::cout << "ref.CoM.x() : " << ref.CoM[timer.loop_count].x() << std::endl;

        // MPCを用いた生成法でtimer.current_count+1番目の目標値を計算
        param.Hp = timer.l_f - timer.loop_count - 1;
        param.Hu = param.Hp;
        if( param.Hu > param.Hp ){
            param.Hu = param.Hp;
        }

        mpc.Init(timer, param ,ref);
        int now_count = timer.loop_count;
        for(int i = 0; i < param.Hp; i++){
          ref.zmp_pgref[i] = zmp_ref_tra[5 * (now_count + i)];
        }

        mpc.Set_System_Mat(com_z_tra, com_ddz_tra);
        state << x_k, dx_k, ddx_k;
        mpc.Update_xk(state);
        mpc.Set_F();
        mpc.Set_G();
        mpc.Set_H();
        ...
        mpc.Set_Wx();
        mpc.Set_Wu();
        mpc.Set_Zref(ref.zmp_pgref);
        // mpc.Set_Xref();
        mpc.Set_Uref();
        mpc.Set_Zconstraint(ref.zmp_pgref);
        mpc.Set_Xconstraint();
        mpc.Set_Uconstraint();
        mpc.Set_qpOASES_mat();
        mpc.QP(&mpc);

        // timer.current_count+1番目の目標値を格納
        x_k = mpc.CoM_PG.x();
        dx_k = mpc.dCoM_PG.x();
        ddx_k = mpc.ddCoM_PG.x();
        dddx_k = mpc.dddCoM_PG.x();
        zmp_k = mpc.ZMP_PG.x();
        
        // timer.current_count+1番目へ更新
        timer.loop_count++;
        timer.current_time = timer.current_time + timer.MPC_dt;


        // 終了条件（c_f-1まで目標値を計算済み）
        if( timer.loop_count == timer.l_f - 1){
    ...
    // 浮遊期(自由落下)（timer.current_time = c_fからスタート）
    while(1){
        ref.CoM[timer.loop_count].x() = ref.x_f + ref.dx_f * (timer.current_time - timer.t_f);
        ref.dCoM[timer.loop_count].x() = ref.dx_f;
        ref.ddCoM[timer.loop_count].x() = 0.0;
        ref.dddCoM[timer.loop_count].x() = 0.0;
        ref.ZMP[timer.loop_count].x() = 0.0;
        // std::cout << "ref.CoM.x() : " << ref.CoM[timer.loop_count].x() << std::endl;
        timer.loop_count++;
        timer.current_time = timer.current_time + timer.MPC_dt;

        // 終了条件（timer.current_count = c_lまで目標値を格納）
        if( timer.loop_count == timer.l_l){
            break;
        }

    }

    // 着地期（timer.current_time = c_l + 1からスタート）
    // timer.current_count番目の目標値
    x_k = ref.x_f + ref.dx_f * (timer.t_l - timer.t_f);
    dx_k = ref.dx_f;
    ddx_k = 0.0;
    dddx_k = 0.0;
    zmp_k = zmp_ref_tra[5 * timer.loop_count];    // 足先位置
    state = Eigen::Vector3d::Zero(3);
    
    while(1){

        std::cout << "MPC2 " << (int)(((timer.current_time - timer.t_l) / (timer.t_re - timer.t_l)) * 100.0) << "% complete" << std::endl;
        ...
        // std::cout << "ref.CoM.x() : " << ref.CoM[timer.loop_count].x() << std::endl;
        // MPCを用いた生成法でtimer.current_count+1番目の目標値を計算
        param.Hp = timer.l_re - timer.loop_count;
        param.Hu = param.Hp;
        if( param.Hu > param.Hp ){
            param.Hu = param.Hp;
        }
        ...
        // 終了条件（timer.current_count = c_eまで目標値を計算済み）
        if( timer.loop_count == timer.l_re){
            ref.CoM[timer.loop_count].x() = x_k;
            ref.dCoM[timer.loop_count].x() = dx_k;
            ref.ddCoM[timer.loop_count].x() = ddx_k;
            ref.dddCoM[timer.loop_count].x() = dddx_k;
    ...
    while(1){
        ...
        ref.CoM[timer.loop_count].x() = ref.x_e;
        ref.ZMP[timer.loop_count].x() = ref.x_e;
        // std::cout << "ref.CoM.x() : " << ref.CoM[timer.loop_count].x() << std::endl;
        timer.loop_count++;
        timer.current_time = timer.current_time + timer.MPC_dt;

        // 終了条件（timer.current_count = c_finish - 1まで目標値を格納済み）
        if( timer.loop_count > timer.l_e){
            ref.CoM[timer.loop_count].x() = ref.x_e;
            ref.ZMP[timer.loop_count].x() = ref.x_e;
    ...
    std::cout << "ref CoM_x generate success" << std::endl;
