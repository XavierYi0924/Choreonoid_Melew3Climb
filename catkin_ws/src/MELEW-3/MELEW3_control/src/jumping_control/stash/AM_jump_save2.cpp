...
void jumping::control_loop(void) {
  ...
  if (phase == 0) {
    if (pose_transition()) {
      ...
      timer.t_js = timer.t_rs + 1.3; // jump start time
  ...
  if(timer.current_count < timer.c_e){
    ...
    delta_x(0) = (ref.CoM[timer.loop_count + 1].x() - ref.CoM[timer.loop_count].x()) / 5;
    delta_x(1) = (ref.dCoM[timer.loop_count + 1].x() - ref.dCoM[timer.loop_count].x()) / 5;
    delta_x(2) = (ref.ddCoM[timer.loop_count + 1].x() - ref.ddCoM[timer.loop_count].x()) / 5;
    delta_x(3) = (ref.ZMP[timer.loop_count + 1].x() - ref.ZMP[timer.loop_count].x()) / 5;

    gene_zmp = Eigen::Vector3d(ref.gene_zmp[timer.current_count].x(), 0.0, 0.0);
    ZMP_pos_ref = Eigen::Vector3d(ref.ZMP[timer.loop_count].x() + timer.delta_count * delta_x(3), 0.0, 0.0);
    COM_pos_ref = Eigen::Vector3d(ref.CoM[timer.loop_count].x() + timer.delta_count * delta_x(0), 0.0, ref.CoM[timer.current_count].z());
    COM_vel_ref = Eigen::Vector3d(ref.dCoM[timer.loop_count].x() + timer.delta_count * delta_x(1), 0.0, ref.dCoM[timer.current_count].z());
    COM_acl_ref = Eigen::Vector3d(ref.ddCoM[timer.loop_count].x() + timer.delta_count * delta_x(2), 0.0, ref.ddCoM[timer.current_count].z());
...
void jumping::XTG(){
    ...
    double x_k, dx_k, ddx_k, dddx_k, am_k, dam_k, ddam_k, zmp_k; 
    x_k = 0.0;
    dx_k = 0.0;
    ddx_k = 0.0;
    dddx_k = 0.0;
    ...
    while(1){
        ...
        ref.AM[timer.loop_count].y() = am_k;
        ref.dAM[timer.loop_count].y() = dam_k;
        ref.ddAM[timer.loop_count].y() = ddam_k;
        ...
        state << x_k, dx_k, ddx_k, am_k, dam_k;
        mpc.Update_xk(state);
        mpc.Set_F();
        mpc.Set_G();
        mpc.Set_H();
    ...
    while(1){
        ...
        ref.AM[timer.loop_count].y() = 0.0;
        ref.dAM[timer.loop_count].y() = 0.0;
        ref.ddAM[timer.loop_count].y() = 0.0;
