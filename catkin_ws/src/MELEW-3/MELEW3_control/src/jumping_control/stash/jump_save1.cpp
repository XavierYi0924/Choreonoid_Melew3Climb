...
void jumping::control_loop(void) {
  ...
  if (phase == 0) {
    if (pose_transition()) {
      ...
      timer.t_js = timer.t_rs + 0.6; // jump start time
      timer.t_sq = timer.t_js + 0.4; // squat time
      timer.t_f = timer.t_sq + 0.3; // flying time
...
void jumping::ZTG(){
  ...
  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    if(period_count < timer.c_js){
      ...
      com_z_tra(period_count) = Get_FreeFall_z(period_count*timer.dt, z2);
      com_dz_tra(period_count) = Get_FreeFall_dz(period_count*timer.dt, z2);
...
void jumping::FTG(){
  ...
  f2 = {timer.t_f + sqrt(2 * param.g * (ref.z_ref - ref.z_f)) / param.g,   
        Get_FreeFall_z(z2.time + sqrt(2 * (ref.z_ref - ref.z_f) / param.g), z2) - ref.z_s, 
        Get_FreeFall_dz(z2.time + sqrt(2 * (ref.z_ref - ref.z_f) / param.g), z2), 
  ...
  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    if(period_count < timer.c_f){
      ...
      ref.FL_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2) - ref.z_s + FL_leg_offset_z_;
      ref.FR_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2) - ref.z_s + FR_leg_offset_z_;
      ref.BL_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2) - ref.z_s + BL_leg_offset_z_;
      ref.BR_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2) - ref.z_s + BR_leg_offset_z_;
    }
    else {
      ref.FL_foot[period_count].x() = ref.gene_zmp[period_count].x() + FL_leg_offset_x_;
      ref.FR_foot[period_count].x() = ref.gene_zmp[period_count].x() + FR_leg_offset_x_;
