...
void jumping::control_loop(void) {
  ...
  if (phase == 0) {
    if (pose_transition()) {
      ...
      ref.z_s = 0.2 * sqrt(2) + 0.136 / 2; // 0.35
      ref.z_f = 0.35 + 0.136 / 2; // 0.418
      ref.z_l = ref.z_s + 0.1;  // 0.45
      ref.z_e = ref.z_s + 0.1;  // 0.45
      ...
      timer.t_sq = timer.t_js + 0.0; // squat time
      timer.t_f = timer.t_sq + 0.5; // flying time
      ...
      timer.l_ini = 0;
      timer.l_s = timer.t_s / timer.MPC_dt; // 800
      timer.l_rs = timer.t_rs / timer.MPC_dt; // 1000
      timer.l_js = timer.t_js / timer.MPC_dt; // 1090
      timer.l_f = timer.t_f / timer.MPC_dt; //1150
      timer.l_l = timer.t_l / timer.MPC_dt; //1188
      timer.l_je = timer.t_je / timer.MPC_dt; //1208
      timer.l_re = timer.t_re / timer.MPC_dt; //1408
      timer.l_e = timer.t_e / timer.MPC_dt; //1608
...
void jumping::ZTG(){
  ...
  // 経由点の情報をまとめたベクトルを定義
  // std::vector<Utils::Five_Spline::point> points_z_1 = {z1, z1_, z2, z3, z4};
  ...
  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    if(period_count < timer.c_js){
      ...
      com_z_tra(period_count) = Get_FreeFall_z(period_count*timer.dt, z2_);
      com_dz_tra(period_count) = Get_FreeFall_dz(period_count*timer.dt, z2_);
...
void jumping::FTG(){
  ...
  f2 = {timer.t_f + sqrt(2 * param.g * (ref.z_ref - ref.z_f)) / param.g,   
        Get_FreeFall_z(z2.time + sqrt(2 * (ref.z_ref - ref.z_f) / param.g), z2_) - ref.z_s, 
        Get_FreeFall_dz(z2.time + sqrt(2 * (ref.z_ref - ref.z_f) / param.g), z2_), 
  ...
  for(int period_count = timer.c_ini; period_count < timer.c_e; period_count++){
    if(period_count < timer.c_f){
      ...
      ref.FL_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2_) - ref.z_s + FL_leg_offset_z_;
      ref.FR_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2_) - ref.z_s + FR_leg_offset_z_;
      ref.BL_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2_) - ref.z_s + BL_leg_offset_z_;
      ref.BR_foot[period_count].z() = Get_FreeFall_z(period_count*timer.dt, z2_) - ref.z_s + BR_leg_offset_z_;
    }
    else {
      ref.FL_foot[period_count].x() = ref.gene_zmp[period_count].x() + FL_leg_offset_x_;
      ref.FR_foot[period_count].x() = ref.gene_zmp[period_count].x() + FR_leg_offset_x_;
