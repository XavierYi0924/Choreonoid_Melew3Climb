...
// VHIPモデルでのMPC重心水平方向パターンジェネレータ（角運動量も考慮）に関するクラス
class VHIP_CAM_MPC_PG{
public:

    Eigen::Vector3d CoM_PG;    // PGで生成するCoM位置
    Eigen::Vector3d dCoM_PG;   // PGで生成するCoM速度
    Eigen::Vector3d ddCoM_PG;  // PGで生成するCoM加速度
    Eigen::Vector3d dddCoM_PG; // PGで生成するCoMジャーク
    Eigen::Vector3d CAM_PG;     // 重心角運動量
    Eigen::Vector3d dCAM_PG;    // 重心角運動量の1回微分
    Eigen::Vector3d ddCAM_PG;   // 重心角運動量の2回微分
    Eigen::Vector3d ZMP_PG;    // PGで生成する（予測）ZMP位置
    ...
    void Update_xk(Eigen::VectorXd state);
    void Set_F();
    void Set_G();
    void Set_H();
    void Set_Wz();
