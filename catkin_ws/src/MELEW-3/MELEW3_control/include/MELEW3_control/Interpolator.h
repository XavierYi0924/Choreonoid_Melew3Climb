...
class Interpolator {
  ...
  struct Sample {
    ...
    Sample(int size) : y(size), yp(size), a(size), a_end(size), b(size), c(size) {}
  };
...
public:
  ...
  double domainLower() const { return samples.empty() ? 0.0 : samples.front().x; }

  double domainUpper() const { return samples.empty() ? 0.0 : samples.back().x; }
